...
import { CompanywiseDocumentService, UserService } from '../../redux/services/feathers/rest.app';
...
const CreateUpdate = () => {
    ...
    const [totalDocumentType, setTotalDocumentType] = useState(0);
    ...
    const openModal = (type, val, data) => {
        setVisible(val);
        setTitle(type);
        setdocumentTypeValues(data);
    };

    const fetchAllDocumentType = async () => {
        if (userData?.user?.companyId) {
            setLoader(true);
            CompanywiseDocumentService.find({
                ...
                .then(async (res) => {
                    ...
                    setTotalDocumentType(res.total);
    ...
    const deleteDocumentType = (values) => {
        setLoader(true);
        CompanywiseDocumentService.remove(values?._id, {
            headers: {
                Authorization: `Bearer ${TokenService.getLocalAccessToken()}`
            }
        })
    ...
    return (
        <div className="page-wrapper card p-3">
            <div className="content container-fluid">
                ...
                <div className="page-header">
                    <div className="row align-items-center">
                        ...
                        <div className="col-auto float-right ml-auto">
                            <button type="button" className="btn btn-primary" onClick={() => openModal('Add Document Type', true, null)}>
                                <i className="pi pi-plus"></i> Add Document Type
                            </button>
                ...
                <SimpleModal title={title} visible={visible} setVisible={() => setVisible(false)} body={<UpdateComponent setVisible={() => setVisible(false)} documentTypeValues={documentTypeValues} fetchAllDocumentType={fetchAllDocumentType} />}></SimpleModal>
                {/* <!-- /Page Header --> */}
                                    ...
                                    {documentTypeList && documentTypeList?.length > 0 ? (
                                        documentTypeList?.map((data, index) => (
                                            <tr key={`key-${index}`}>
                                                <td>{data?.name}</td>
                                                <td>
                                                    <button className="btn btn-primary" type="button" onClick={() => openModal('Update Document Type', true, data)}>
                                                        <i className="pi pi-pencil"></i>
                                                    </button>
                                                    &nbsp;
                                                    <button className="btn btn-warning" type="button" onClick={() => deleteDocumentType(data)}>
                                    ...
                                    documentTypeList?.map((data, index) => (
                                        <div className="col-md-3 mb-3">
                                            <div className="card dash-widget pb-2 pt-3" key={`key-${index}`}>
                                                ...
                                                <div className='mt-3'>
                                                    <button className="btn-outline-delete" onClick={() => deleteDocumentType(data)}  > <i className="pi pi-trash" style={{ marginRight: "6px" }}></i>Delete</button>
                                                    <button className="btn-outline-edit" onClick={() => openModal('Update Document Type', true, data)}>  <i className="pi pi-pencil" style={{ marginRight: "4px" }}></i> Edit</button>
