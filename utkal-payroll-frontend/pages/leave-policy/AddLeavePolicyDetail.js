...
import { leavePolicyDetailSchema } from '../../redux/helpers/validations';
...
const AddLeavePolicyDetail = (props) => {
    ...
    const addUpdate = async (values) => {
        ...
        if (props?.componentValues !== null) {
            ...
            const updatedLeaveDetails = leaveDetails.map((detail) => {
            ...
            const leaveRecord = await LeavePolicyService.find({
                query: queryData,
                headers: {
                    Authorization: `Bearer ${TokenService.getLocalAccessToken()}`
                }
            });
            existingLeaveDetails = leaveRecord?.data[0]?.leaveDetails || [];
            existingLeaveDetails.push(data);
            setLoader(true);
            LeavePolicyService.patch(props?.policyId,
                {
                    leaveDetails: existingLeaveDetails
                }, {
                headers: {
                    Authorization: `Bearer ${TokenService.getLocalAccessToken()}`
                }
    ...
    return (
        <Formik
            ...
            validationSchema={leavePolicyDetailSchema}
            onSubmit={(values) => {
