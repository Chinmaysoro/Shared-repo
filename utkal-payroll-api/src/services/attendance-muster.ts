...
import generateAttendanceReport from './../hooks/generateAttendanceReport';
// Separate function to generate attendance report for a user


const attendanceMuster = (app) => {
  app.use('/v1/attendance-muster', {
    async find(params) {
      ...
      const result = await generateAttendanceReport(app, companyId, startDate, endDate, userData);
      return result;
    }
  });

  const service = app.service('v1/attendance-muster');
